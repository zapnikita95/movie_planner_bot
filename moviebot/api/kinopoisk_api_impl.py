"""
API реализация для работы с Kinopoisk API Unofficial (kinopoiskapiunofficial.tech)
Этот файл содержит исходную реализацию API.
Используется через api_manager.py для поддержки fallback.
"""
import re
import requests
import logging
...
def get_seasons(kp_id, chat_id=None, user_id=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if seasons:
                # Получаем информацию о просмотренных сериях (в группе — общий прогресс по чату)
            ...
            if response.status_code == 200:
                ...
                seasons = data.get('items', [])
                if seasons:
                    # Получаем информацию о просмотренных сериях (в группе — общий прогресс)
                    watched_episodes = set()
                    if chat_id:
                        with db_lock:
                            cursor.execute('SELECT id FROM movies WHERE chat_id = %s AND kp_id = %s', (chat_id, str(kp_id)))
