...
# ID владельца бота (получаем из переменной окружения или используем ID бота)
...
def get_bot_owner_id():
    ...
    if BOT_OWNER_ID is None:
        try:
            ...
            # Владелец бота - это тот, кто запустил бота (можно задать через переменную окружения)
            import os
            owner_id_str = os.getenv('BOT_OWNER_ID')
            if owner_id_str:
                BOT_OWNER_ID = int(owner_id_str)
            else:
                # Если не задан, используем ID первого пользователя, который отправил /promo
                # В реальности лучше задать через переменную окружения
                logger.warning("BOT_OWNER_ID не задан в переменных окружения. Команда /promo будет доступна только после первого использования.")
        except Exception as e:
            logger.error(f"Ошибка при получении информации о боте: {e}", exc_info=True)
    return BOT_OWNER_ID


def register_promo_handlers(bot_instance):
    """Регистрирует обработчики команды /promo"""
    ...
    def promo_command(message):
        ...
        try:
            ...
            # Для простоты проверяем, что это личное сообщение
            # В реальности нужно добавить проверку на владельца через переменную окружения
