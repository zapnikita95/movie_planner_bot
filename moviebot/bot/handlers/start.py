...
Выберите раздел из меню ниже ⬇
    ...
    try:
        ...
        # Сначала обычные кнопки до Рандома
...
Выберите раздел из меню ниже ⬇
        ...
        try:
            bot.edit_message_text(welcome_text, chat_id, call.message.message_id, reply_markup=markup, parse_mode='HTML')
        except:
...
# Функция для совместимости с main.py (можно оставить, но она теперь ничего не делает)
def register_start_handlers(bot_instance):
    logger.info("[START] register_start_handlers вызвана (handlers уже зарегистрированы глобально)")
    # Ничего не делаем — обработчики зарегистрированы при импорте модуля